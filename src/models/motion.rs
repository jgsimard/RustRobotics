...
impl MotionModel<f32, 3, 2, 2> for Velocity {
    fn prediction(&self, x: &Vector3<f32>, u: &Vector2<f32>, dt: f32) -> Vector3<f32> {
        //state
        let theta = x[2];
        //control
        let v = u[0];
    ...
    fn jacobian_wrt_state(&self, x: &Vector3<f32>, u: &Vector2<f32>, dt: f32) -> Matrix3<f32> {
        //state
        let theta = x[2];
        //control
        let v = u[0];
    ...
    #[allow(clippy::deprecated_cfg_attr)]
    fn jacobian_wrt_input(&self, x: &Vector3<f32>, u: &Vector2<f32>, dt: f32) -> Matrix3x2<f32> {
        //state
        let theta = x[2];
        //control
        let v = u[0];
...
impl MotionModel<f32, 4, 2, 2> for SimpleProblemMotionModel {
    fn prediction(&self, x: &Vector4<f32>, u: &Vector2<f32>, dt: f32) -> Vector4<f32> {
        let yaw = x[2];
        let v = x[3];
        Vector4::new(
            x.x + yaw.cos() * v * dt,
    ...
    #[allow(clippy::deprecated_cfg_attr)]
    fn jacobian_wrt_state(&self, x: &Vector4<f32>, u: &Vector2<f32>, dt: f32) -> Matrix4<f32> {
        let yaw = x[2];
        let v = u[0];
        #[cfg_attr(rustfmt, rustfmt_skip)]
            Matrix4::<f32>::new(
    ...
    fn jacobian_wrt_input(&self, _x: &Vector4<f32>, _u: &Vector2<f32>, _dt: f32) -> Matrix4x2<f32> {
        unimplemented!()
    }
}
